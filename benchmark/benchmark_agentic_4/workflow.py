...
全部参数写在 benchmark/benchmark_agentic_4/config.yaml::
...
logger = logging.getLogger("benchmark_agentic_4.workflow")


class Agentic4BenchmarkWorkflow:
    ...
    ) -> "Agentic4BenchmarkWorkflow":
        cfg = BenchmarkConfig.from_yaml(config_path)
        return cls(cfg)

    def _setup_logging(self, level: int):
        root_logger = logging.getLogger("benchmark_agentic_4")
    ...
    def generate_questions(self, save: bool = True) -> Dict[str, Any]:
        """步骤 1：从 doc 表指定范围抽样，按 type_counts 出题；每完成一题实时落盘。"""
        self.setup_llm()
        gen = QuestionGenerator(
            self.llm,
            model_args=self.cfg.gen_model_args,
            db_path=self.cfg.db_path,
            type_counts=self.cfg.type_counts,
            seed=self.cfg.seed,
            ...
            doc_id_range=self.cfg.doc_id_range,
            n_limit=self.cfg.n_limit,
            timeout=self.cfg.gen_timeout,
        )
        out = self.cfg.questions_file() if save else None
        existing = []
        if save and self.cfg.gen_resume and out is not None and out.is_file():
            try:
    ...
    def evaluate(
        ...
        existing = []
        if save and self.cfg.eval_resume and out is not None and out.is_file():
            try:
                prev = self.load_json(out)
    ...
    def report(
        ...
        *,
        ...
        for k in (
            "db_path", "type_counts", "seed", "query_mode", "judge_model",
    ...
    def run(self) -> Any:
        ...
        print(
            f"[benchmark_agentic_4] mode={mode} config={self.cfg.config_file}",
            file=sys.stderr,
        )
        if mode == "generate":
        ...
        raise ValueError(
            ...
            f"Set run.mode in benchmark/benchmark_agentic_4/config.yaml: "

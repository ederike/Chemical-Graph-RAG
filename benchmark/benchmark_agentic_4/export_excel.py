...
def _hop_keys(by_question_type: Dict[str, Any]) -> List[str]:
    def key(h: str):
        try:
            return (0, int(h))
        except (TypeError, ValueError):
    ...
    return sorted((by_question_type or {}).keys(), key=key)
...
def _add_toc(wb: Workbook, items: Sequence[Tuple[str, str]]) -> None:
    ...
    _write_title(ws, "benchmark_agentic_4 评测报告", 2)
...
def _add_meta(
    ...
    preferred = [
        ...
        "eval_elapsed_s", "elapsed_s", "db_path", "type_counts", "seed",
        "doc_id_range", "source_path", "config_file", "dhmf_config_path",
    ]
    seen = set()
    for k in preferred:
        if k in meta:
...
def _add_by_question_type(wb: Workbook, summary: Dict[str, Any]) -> Optional[str]:
    systems = _systems(summary)
    hop_maps: Dict[str, Dict[str, Any]] = {}
    hops: List[str] = []
    seen = set()
    for s in systems:
        bh = (summary.get(s) or {}).get("by_question_type") or {}
    ...
    _write_title(ws, "按题型切分：Agentic vs 纯LLM" if has_pair else "按题型切分", len(headers))
    start = 3
    _write_table(
    ...
    chart_headers = ["题型"] + [_sys_label(s) for s in systems]
    _write_table(
...
def _add_details(
    ...
    fail_headers = [
        "ID", "题型", "系统", "问题", "query状态", "query错误", "judge状态", "judge错误",
    ]
    for r in results:
        for system in systems:
            ...
            if qerr or jerr or qs == 0 or js == 0:
                fail_rows.append([
                    r.get("id"), r.get("question_type"), _sys_label(system),
...
def write_report_workbook(
    ...
    *,
    ...
    hop_name = _add_by_question_type(wb, summary)
    if hop_name:
        toc.append((hop_name, "按题型正确率、三路检索延迟、改写/重排/预加载"))

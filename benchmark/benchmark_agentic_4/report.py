...
def _summarize_one(
    ...
    *,
    ...
    for r in rows:
        type_groups[r.get("question_type")].append(r)
    for qtype, group in sorted(type_groups.items(), key=lambda x: (x[0] is None, str(x[0] or ""))):
        ...
        for k in RETRIEVE_STAGE_KEYS:
            type_item[f"mean_{k}"] = mean(_metric_vals(group, k))
            type_item[f"mean_{k}_per_search"] = mean(_metric_vals(group, f"mean_{k}"))
        if enable_doc_recall:
            g_recalls = [
            ...
            type_item["mean_doc_recall"] = mean(g_recalls)
        by_question_type[str(qtype)] = type_item
    ...
    *,
    ...
    return {
        "meta": {
            ...
            "type_counts": slim_ds.get("type_counts") or cfg_src.get("type_counts"),
            "seed": slim_ds.get("seed", cfg_src.get("seed")),
            "doc_id_range": slim_ds.get("doc_id_range", cfg_src.get("doc_id_range")),

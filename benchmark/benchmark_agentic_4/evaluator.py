...
class QueryEvaluator:
    ...
    def _collect_agentic_metrics(respond: Dict[str, Any], wall_s: float) -> Dict[str, Any]:
        ...
        # 每 turn 平均墙钟（若有 wall / n_turns）
        try:
            n_turns = int(metrics.get("n_turns") or 0)
        except (TypeError, ValueError):
            n_turns = 0
        if n_turns > 0 and wall_s is not None:
            try:
                metrics["mean_turn_s"] = float(wall_s) / n_turns
            except (TypeError, ValueError):
                metrics["mean_turn_s"] = None
        else:
            metrics["mean_turn_s"] = None

        # 若系统返回 multi-path / multipath timing，原样保留并做简单聚合
        multi = (
            respond.get("multi_path_timing")
            or respond.get("multipath_timing")
            or respond.get("multi_path")
            or timing.get("multi_path")
            or timing.get("paths")
        )
        metrics["multi_path_timing"] = multi if isinstance(multi, (dict, list)) else None
        if isinstance(multi, dict):
            path_vals = []
            for _pk, pv in multi.items():
                if isinstance(pv, dict):
                    for kk in ("total_s", "latency_s", "retrieve_s"):
                        if pv.get(kk) is not None:
                            try:
                                path_vals.append(float(pv[kk]))
                            except (TypeError, ValueError):
                                pass
                            break
                else:
                    try:
                        path_vals.append(float(pv))
                    except (TypeError, ValueError):
                        pass
            metrics["multi_path_n"] = len(multi)
            metrics["multi_path_mean_s"] = (
                sum(path_vals) / len(path_vals) if path_vals else None
            )
            metrics["multi_path_sum_s"] = sum(path_vals) if path_vals else None
            ...
            metrics["multi_path_n"] = None
            metrics["multi_path_mean_s"] = None
            metrics["multi_path_sum_s"] = None
        return metrics
    ...
    def evaluate_one(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = {
            ...
            "hop": item.get("hop"),
            "question": question,
            "gold_answer": item.get("gold_answer") or item.get("ground_truth_answer"),
            "entities": item.get("entities") or {},
            "ground_truth_answer": item.get("ground_truth_answer") or item.get("gold_answer") or "",

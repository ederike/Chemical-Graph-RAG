"""benchmark_agentic_4 通用工具：路径、文档抽样、LLM 调用、JSON 解析。"""
...
def project_root() -> Path:
    """仓库根目录：benchmark/benchmark_agentic_4/ 上两级。"""
...
def parse_doc_id_range(raw: Any) -> Tuple[Optional[int], Optional[int]]:
    ...
    解析 doc 表 id 闭区间。
      ...
      null / 省略 / "" → (None, None)  不限制
      N                 → (0, N)        0 <= id <= N（含 0 与 N）
      [a, b]            → (a, b)        a <= id <= b
    """
    if raw is None or raw == "" or raw is False:
        return None, None

    def _as_int(v: Any, *, label: str) -> int:
        if isinstance(v, bool):
            raise ValueError(
                f"doc_id_range 非法: {label} 不能为 bool，得到 {v!r}"
            )
        try:
            return int(v)
        except (TypeError, ValueError) as e:
            raise ValueError(
                f"doc_id_range 非法: {label} 无法转为整数，得到 {v!r}"
            ) from e

    if isinstance(raw, dict):
        raise ValueError(
            "doc_id_range 不支持 dict；请使用 null、单个数字 N，或 [a, b]"
        )

    if isinstance(raw, (list, tuple)):
        if len(raw) != 2:
            raise ValueError(
                f"doc_id_range 列表必须恰好 2 个元素 [a, b]，得到 {raw!r}"
            )
        a = _as_int(raw[0], label="a")
        b = _as_int(raw[1], label="b")
        if a > b:
            raise ValueError(f"doc_id_range 要求 a <= b，得到 [{a}, {b}]")
        return a, b

    n = _as_int(raw, label="N")
    if n < 0:
        raise ValueError(f"doc_id_range 单个数字 N 不能为负，得到 {n}")
    return 0, n


def parse_type_counts(raw: Any) -> Dict[str, int]:
    """规范化五类题型数量配置；值为 0 的类型可省略。"""
    allowed = {
        "product_to_formula",
    ...
    for k, v in (raw or {}).items():
        ...
        if key not in allowed:
            raise ValueError(f"未知 question_type={key!r}；允许: {sorted(allowed)}")
        n = int(v)
        if n < 0:
            raise ValueError(f"问题数量不能为负: type={key} n={n}")
        if n > 0:
            out[key] = n
    if not out:
        raise ValueError("generate.type_counts 为空，请至少配置一类题的数量 > 0")
    return out
...
def _doc_range_clause(
    ...
    """按闭区间 lo–hi 限制 doc.id；lo/hi 为 None 表示该端不限制。"""
    sql = (
...
def load_doc_index_from_db(
    ...
    *,
    ...
    if not docs:
        lo, hi = doc_id_min, doc_id_max
        raise RuntimeError(
            f"doc 表无可用 content: {resolve_path(db_path)} "
            f"id∈[{lo if lo is not None else '*'}, {hi if hi is not None else '*'}]"
        )
    return docs


def count_docs_matching(
    db_path: Union[str, Path],
    *,
    doc_id_min: Optional[int] = None,
    doc_id_max: Optional[int] = None,
) -> int:
    """统计范围内的候选文档数（不加载全表到内存）。"""
    where, params = _doc_range_clause(doc_id_min, doc_id_max)
    sql = "SELECT COUNT(*) AS n FROM doc" + where
    conn = _connect_doc_db(db_path)
    try:
